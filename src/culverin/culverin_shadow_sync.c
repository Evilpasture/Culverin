...
static CULV_FORCE_INLINE void process_full_batch(PhysicsWorldObject *self,
  ...
  auto *CULV_RESTRICT s_pos =
      (PosStride *)CULV_ASSUME_ALIGNED(self->positions, 32);
  auto *CULV_RESTRICT s_ppos =
      (PosStride *)CULV_ASSUME_ALIGNED(self->prev_positions, 32);
  auto *CULV_RESTRICT s_rot =
      (AuxStride *)CULV_ASSUME_ALIGNED(self->rotations, 16);
  auto *CULV_RESTRICT s_prot =
      (AuxStride *)CULV_ASSUME_ALIGNED(self->prev_rotations, 16);
  auto *CULV_RESTRICT s_lvel =
      (AuxStride *)CULV_ASSUME_ALIGNED(self->linear_velocities, 16);
  auto *CULV_RESTRICT s_avel =
      (AuxStride *)CULV_ASSUME_ALIGNED(self->angular_velocities, 16);
...
// =================================================================================================
static void process_partial_batch(PhysicsWorldObject *self,
                                  const SyncWorkItem *worklist,
                                  uint32_t count) {
  if (count == 0) {
    return;
