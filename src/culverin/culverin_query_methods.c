...
PyObject *PhysicsWorld_raycast(PhysicsWorldObject *self, PyObject *args,
  ...
  if (has_hit) {
    ...
    hit_handle = (BodyHandle)JPH_BodyInterface_GetUserData(self->body_interface,
                                                           hit->bodyID);
  ...
  if (slot < self->slot_capacity && self->generations[slot] == gen &&
    ...
    result = Py_BuildValue("Kf(fff)", hit_handle, hit_fraction, normal.x,
                           normal.y, normal.z);
  }
...
PyObject *PhysicsWorld_raycast_batch(PhysicsWorldObject *self,
  ...
  if (LIKELY(kwnames == NULL && (nargs == 2 || nargs == 3))) {
    if (UNLIKELY(PyObject_GetBuffer(args[0], &b_starts, PyBUF_SIMPLE) < 0)) {
      return NULL;
    }
    ...
    int ok = PyArg_ParseTupleAndKeywords(temp_tuple, temp_dict, "y*y*|f",
                                         kwlist, &b_starts, &b_dirs, &max_dist);
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (JPH_NarrowPhaseQuery_CastRay(query, &v_ori, &v_dir, &hit, NULL, NULL,
                                     NULL)) {
...
PyObject *PhysicsWorld_shapecast(PhysicsWorldObject *self, PyObject *args,
  ...
  static char *const kwlist[] = {"shape", "pos",    "rot", "dir",
                           "size",  "ignore", NULL};

  // 1. ARGUMENT PARSING (GIL HELD)
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "i("JPH_REAL_STRING JPH_REAL_STRING JPH_REAL_STRING")(ffff)(fff)O|K", kwlist,
  ...
  if (ctx.has_hit) {
    uint64_t h_raw =
        JPH_BodyInterface_GetUserData(self->body_interface, ctx.hit.bodyID2);
    hit_handle = h_raw;
    has_valid_hit = true;
  }
  ...
  if (has_valid_hit) {
    ...
    if (slot < self->slot_capacity && self->generations[slot] == gen &&
      ...
      if (!handle_obj || !fraction_obj || !px || !py || !pz || !nx_obj ||
          !ny_obj || !nz_obj) {
        Py_XDECREF(handle_obj);
        Py_XDECREF(fraction_obj);
        Py_XDECREF(px);
        Py_XDECREF(py);
        Py_XDECREF(pz);
        Py_XDECREF(nx_obj);
