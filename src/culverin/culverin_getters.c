...
PyObject *Vehicle_get_wheel_count(VehicleObject *self, void *Py_UNUSED(closure)) {
  // num_wheels is set at creation and never changes
  return PyLong_FromUnsignedLong(self->num_wheels);
}

PyObject *Character_get_handle(CharacterObject *self, void *Py_UNUSED(closure)) {
...
PyObject *get_count(PhysicsWorldObject *self, void *Py_UNUSED(c)) {
  SHADOW_LOCK(&self->shadow_lock);
...
PyObject *get_time(PhysicsWorldObject *self, void *Py_UNUSED(c)) {
  SHADOW_LOCK(&self->shadow_lock);
...
PyObject *get_user_data_buffer(PhysicsWorldObject *self, void *Py_UNUSED(c)) {
  // User data is Stride 1 (One uint64 per body)
  return make_view(self, self->user_data, "Q", sizeof(uint64_t), 1);
}

PyObject *get_shape_count(PhysicsWorldObject *self, void *Py_UNUSED(closure)) {
