...
void culv_jph_trace(const char* const inString) {
    ...
    const size_t write_size = (result.size > buffer.size()) ? buffer.size() : result.size;
    std::string_view output_view(buffer.data(), write_size);
    
    std::cerr << output_view;
}
...
auto culv_jph_assert(const char* const inExpression, const char* const inMessage, 
    ...
    const size_t write_size = (result.size > buffer.size()) ? buffer.size() : result.size;
    std::cerr << std::string_view(buffer.data(), write_size);
    std::cerr.flush();
