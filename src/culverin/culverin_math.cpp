...
void culverin_math_get_perspective(float fovy_rad, float aspect, float near_p, float far_p,
    ...
    m.SetColumn4(2, JPH::Vec4(0.0f, 0.0f, (far_p + near_p) / (near_p - far_p),
                              (2.0f * far_p * near_p) / (near_p - far_p)));
...
void culverin_math_get_ortho(float left, float right, float bottom, float top, float near_p,
    ...
    m.SetColumn4(
        2, JPH::Vec4(0.0f, 0.0f, -2.0f / (far_p - near_p), -(far_p + near_p) / (far_p - near_p)));
...
void culverin_math_get_look_at(const float *__restrict eye, const float *__restrict target,
    ...
    // 1. Load into Jolt types.
    // Jolt's Vec3 is actually a Vec4 internally for SIMD alignment.
    JPH::Vec3 e(eye[0], eye[1], eye[2]);
    JPH::Vec3 t(target[0], target[1], target[2]);
    JPH::Vec3 u(up[0], up[1], up[2]);

    // 2. Standard Look-At Math
    JPH::Vec3 f     = (t - e).NormalizedOr(JPH::Vec3::sAxisZ()); // Handle zero-length
    JPH::Vec3 r     = f.Cross(u).NormalizedOr(JPH::Vec3::sAxisX());
    JPH::Vec3 u_new = r.Cross(f);

    // 3. Construct Matrix
    JPH::Mat44 m;
    m.SetColumn4(0, JPH::Vec4(r.GetX(), u_new.GetX(), -f.GetX(), 0.0f));
    m.SetColumn4(1, JPH::Vec4(r.GetY(), u_new.GetY(), -f.GetY(), 0.0f));
    m.SetColumn4(2, JPH::Vec4(r.GetZ(), u_new.GetZ(), -f.GetZ(), 0.0f));
    m.SetColumn4(3, JPH::Vec4(-r.Dot(e), -u_new.Dot(e), f.Dot(e), 1.0f));

    // 4. THE FIX: Safe Store
    // If 'out' isn't guaranteed 16-byte aligned, use the unaligned store
    // or ensure the caller used alignas(16).
    m.StoreFloat4x4(reinterpret_cast<JPH::Float4 *>(out));
}
...
// -----------------------------------------------------------------------------
void culverin_math_get_trs_batch(size_t count,
