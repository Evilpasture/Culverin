...
static inline void manual_vec3_rotate_by_quat(const JPH_Vec3 *CULV_RESTRICT v,
                                              const JPH_Quat *CULV_RESTRICT q,
                                              JPH_Vec3 *CULV_RESTRICT out) {
...
static inline void manual_quat_multiply(const JPH_Quat *CULV_RESTRICT a,
                                        const JPH_Quat *CULV_RESTRICT b,
                                        JPH_Quat *CULV_RESTRICT out) {
...
static inline void manual_mat4_from_quat(const JPH_Quat *CULV_RESTRICT q,
                                         JPH_Mat4 *CULV_RESTRICT out) {
    float x2 = q->x + q->x;
    float y2 = q->y + q->y;
    float z2 = q->z + q->z;
...
static inline void manual_mat4_multiply(const JPH_Mat4 *CULV_RESTRICT A,
                                        const JPH_Mat4 *CULV_RESTRICT B,
                                        JPH_Mat4 *CULV_RESTRICT out) {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        // out_col_i = A * b_col_i
        out->column[i].x =
            A->column[0].x * x + A->column[1].x * y + A->column[2].x * z + A->column[3].x * w;
        out->column[i].y =
            A->column[0].y * x + A->column[1].y * y + A->column[2].y * z + A->column[3].y * w;
        out->column[i].z =
            A->column[0].z * x + A->column[1].z * y + A->column[2].z * z + A->column[3].z * w;
        out->column[i].w =
            A->column[0].w * x + A->column[1].w * y + A->column[2].w * z + A->column[3].w * w;
    }
}