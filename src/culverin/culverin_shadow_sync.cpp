...
// =================================================================================================
template <JPH::EBodyType TType>
[[gnu::always_inline, gnu::flatten, gnu::nonnull(2, 3)]] inline auto
execute_sync_pass(const uint32_t active_count, const JPH::PhysicsSystem *const CULV_RESTRICT system,
                  const PhysicsWorldObject *const CULV_RESTRICT self,
                  const WorldDataCreateInfo world) noexcept -> void {
    if (active_count == 0) {
        return;
    }
    ...
    for (uint32_t i = 0; i < active_count; i++) {
        ...
        const uint32_t state_bad = [state]() -> auto {
            if constexpr (TType == JPH::EBodyType::RigidBody) {
                return (state == SLOT_ALIVE || state == SLOT_CHARACTER) ? 0 : 1;
            } else {
                return (state == SLOT_SOFT_BODY) ? 0 : 1;
            }
        }();

        const uint32_t bad =
            static_cast<const uint32_t>(slot >= slot_capacity) | (current_gen ^ gen) | state_bad;
        const uint32_t d_idx = s2d[safe_slot];
        const auto is_valid  = static_cast<uint32_t>(bad == 0);

        CULV_PREFETCH_WRITE(&world.shadow_pos[d_idx]);
        CULV_PREFETCH_WRITE(&world.shadow_rot[d_idx]);
        if constexpr (TType == JPH::EBodyType::SoftBody) {
            CULV_PREFETCH_READ(&world.soft_shadows[d_idx]);
        }
...
culverin_sync_shadow_buffers(const PhysicsWorldObject *const CULV_RESTRICT self) noexcept -> void {
    ...
    const WorldDataCreateInfo world = {
        .shadow_pos = reinterpret_cast<PosStride *const CULV_RESTRICT>(
            CULV_ASSUME_ALIGNED(self->positions, sizeof(PosStride))),
        .shadow_ppos = reinterpret_cast<PosStride *const CULV_RESTRICT>(
            CULV_ASSUME_ALIGNED(self->prev_positions, sizeof(PosStride))),
        .shadow_rot = reinterpret_cast<AuxStride *const CULV_RESTRICT>(
            CULV_ASSUME_ALIGNED(self->rotations, sizeof(AuxStride))),
        .shadow_prot = reinterpret_cast<AuxStride *const CULV_RESTRICT>(
            CULV_ASSUME_ALIGNED(self->prev_rotations, sizeof(AuxStride))),
        .shadow_lvel = reinterpret_cast<AuxStride *const CULV_RESTRICT>(
            CULV_ASSUME_ALIGNED(self->linear_velocities, sizeof(AuxStride))),
        .shadow_avel = reinterpret_cast<AuxStride *const CULV_RESTRICT>(
            CULV_ASSUME_ALIGNED(self->angular_velocities, sizeof(AuxStride))),
        .soft_shadows = self->soft_shadows};

    execute_sync_pass<EBodyType::RigidBody>(active_rigid_count, system, self, world);

    if (self->soft_shadows != nullptr) {
        execute_sync_pass<EBodyType::SoftBody>(active_soft_count, system, self, world);
    }
