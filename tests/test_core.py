...
class TestTupleHacks(CulverinTestCase):
    """
    Validation for the C-level 'mutate_tuple' engine.
    Verifies that we can bypass Python's immutability for high-performance metadata updates.
    """

    def test_in_place_mutation(self):
        """Verify that index swapping occurs without changing the object's address."""
        # 1. Setup victim tuple
        t = (100.5, 200.5, 300.5)
        original_id = id(t)
        original_hash = hash(t)

        # 2. Mutate index 1 (3-argument mode)
        # Signature: (target, index, value)
        new_hash_returned = culverin.mutate_tuple(t, 1, "MODIFIED")

        # 3. Assertions
        self.assertEqual(id(t), original_id, "Memory address changed! Mutate failed to be in-place.")
        self.assertEqual(t[1], "MODIFIED", "The value at index 1 was not updated.")
        self.assertNotEqual(hash(t), original_hash, "The hash was not recomputed.")
        self.assertEqual(hash(t), new_hash_returned, "The returned hash doesn't match hash(t).")

    def test_negative_indexing(self):
        """Verify that the C-layer correctly handles Python-style negative indices."""
        t = (1, 2, 3, 4)
        culverin.mutate_tuple(t, -1, 999)
        self.assertEqual(t[3], 999)
        
        culverin.mutate_tuple(t, -4, 111)
        self.assertEqual(t[0], 111)

    def test_atomic_dictionary_migration(self):
        """
        Verify the 'Atomic Migration' (5-argument mode).
        Ensures a dictionary remains functional after its keys change value/hash.
        """
        # 1. Put a tuple in a dictionary
        key_name = "physics_body_01"
        meta_tuple = (0.0, 1.0, "ACTIVE")
        registry = {key_name: meta_tuple}

        # 2. Mutate the tuple via the atomic path
        # Signature: (target, index, value, registry_dict, key)
        # This performs: pop -> mutate -> rehash -> push entirely in C
        culverin.mutate_tuple(meta_tuple, 2, "SLEEPING", registry, key_name)

        # 3. Verification
        # If the rehash/re-index failed, the dictionary would return None for this key
        self.assertEqual(registry[key_name][2], "SLEEPING")
        
        # Verify it can be found via standard lookup (Bucket check)
        self.assertIn(key_name, registry)
        self.assertEqual(registry.get(key_name), meta_tuple)
        self.assertEqual(len(registry), 1, "Registry size should not have changed.")

    def test_mutation_errors(self):
        """Verify safety guards against mismatched keys or invalid types."""
        # Use floats and force a copy to ensure different object IDs (id())
        t = (1000.1, 2000.1)
        registry = {"correct_key": t}

        # 1. Out of bounds
        with self.assertRaises(IndexError):
            culverin.mutate_tuple(t, 5, "oops")

        # 2. Wrong type
        with self.assertRaises(TypeError):
            culverin.mutate_tuple([1, 2], 0, "not a tuple") # type: ignore

        # 3. Identity Mismatch (Safety Guard)
        # Force other_tuple to be a distinct object with the same content
        other_tuple = tuple(list(t)) 
        
        # Sanity check for the test itself: pointers must be different
        assert id(t) != id(other_tuple) 

        with self.assertRaises(ValueError) as cm:
            # We pass 'other_tuple', but the registry contains 't'
            culverin.mutate_tuple(other_tuple, 0, 99.9, registry, "correct_key")
        self.assertIn("not the same object", str(cm.exception))

        # 4. Unhashable error
        # Putting a list inside a tuple makes the tuple unhashable
        with self.assertRaises(TypeError):
            culverin.mutate_tuple(t, 0, [1, 2, 3])

    def test_high_frequency_registry_stress(self):
        """Simulate high-speed metadata updates typical of a physics loop."""
        registry = {f"body_{i}": (0.0, 0.0) for i in range(100)}
        
        # Snapshot one to track
        target_key = "body_50"
        target_tuple = registry[target_key]
        
        # Rapidly mutate the velocity index
        for i in range(1000):
            culverin.mutate_tuple(target_tuple, 0, float(i), registry, target_key)
            
        self.assertEqual(registry[target_key][0], 999.0)
