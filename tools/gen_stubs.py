...
class StubGenerator:
    ...
    def _infer_return_type(self, method_name: str) -> str:
        m = method_name.lower()
        if any(x in m for x in ("create", "get_index", "count", "get_joint")):
            return "int"
        if any(x in m for x in ("is_", "has_", "apply_buoyancy")):
            return "bool"
        if any(x in m for x in ("raycast", "shapecast", "overlap")):
    ...
    def _infer_prop_type(self, prop_name: str) -> str:
        p = prop_name.lower()
        if p in ("positions", "rotations", "velocities", "angular_velocities", "user_data"):
            return "memoryview"
        if p in (
            "count",
            "shape_count",
            ...
            return "int"
        if p in ("time", "friction", "restitution"):
            return "float"
        if p.startswith("is_"):
            return "bool"
        return "Any"
    ...
    def get_class_methods(self) -> dict[str, list[MethodInfo]]:
        ...
        method_pattern = re.compile(
            r"static\s+(?:const\s+)?PyMethodDef\s+(\w+)_methods\s*\[\]\s*=\s*\{(.*?)\}\s*;",
            re.DOTALL,
        )
        getset_pattern = re.compile(
            r"static\s+(?:const\s+)?PyGetSetDef\s+(\w+)_getset\s*\[\]\s*=\s*\{(.*?)\}\s*;",
            re.DOTALL,
        )
